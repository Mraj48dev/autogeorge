...
export class DalleImageGenerationService implements ImageGenerationService {
  ...
  async generateImage(request: ImageGenerationRequest): Promise<Result<GeneratedImageResponse, ImageGenerationError>> {
    try {
      // Determine prompt based on generation mode
      let prompt: string;

      if (request.generationMode === 'manual' || !request.generationMode) {
        // Legacy behavior: build prompt manually
        prompt = this.buildPrompt(request.title, request.content, request.aiPrompt);
      } else {
        // AI-assisted or full-auto: should use aiPrompt directly (pre-generated by PromptEngineer)
        if (!request.aiPrompt) {
          return Result.fail({
            code: 'INVALID_PROMPT',
            message: `AI-generated prompt is required for ${request.generationMode} mode`,
          });
        }
      ...
      console.log(`🎨 [DALL-E] Mode: ${request.generationMode || 'manual'}`);
  ...
  async generateImageWithAiPrompt(
    request: ImageGenerationRequest,
    aiGeneratedPrompt: string
  ): Promise<Result<GeneratedImageResponse, ImageGenerationError>> {
    // Override the aiPrompt with the AI-generated one and call regular generateImage
    const enhancedRequest = {
      ...request,
      aiPrompt: aiGeneratedPrompt,
      generationMode: request.generationMode || 'ai_assisted'
    };
