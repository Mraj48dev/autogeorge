...
export class FeedPollingService {
  ...
  async pollSingleFeed(source: Source): Promise<Result<PollingResult, string>> {
    ...
    try {
      ...
      // Sources Module è ora completamente disaccoppiato - solo raccoglie dati
      if (newItems > 0) {
        console.log(`📥 Saved ${newItems} new feed items for ${sourceName}`);
        console.log(`🔄 Articles will be generated by separate cron job`);
      }
